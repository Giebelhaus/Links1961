...
// The second set is grabbed a specified time later. The distance between the two sets is calculated and stored in a form field.
...
function timedMousePos() {
	...
	if (xPos >= 0 && yPos >= 0) {
		// 0,0 is a valid mouse position, so I want to accept that.  for this reason my vars are initialized to -1
		var newX = xPos;
		var newY = yPos;
		intervals++;
	}
...
function calcDistance(aX,aY,bX,bY) {
	// use the Euclidean 2 dimensional distance formula to calculate the distance in pizels between the coordinate sets 
	var mouseTraveled = Math.round(Math.sqrt(Math.pow(aX-bX,2)+Math.pow(aY-bY,2)));
	// ajax stuff to set a session variable
	try	{
		// try/catch for giant pages that take a while to 
